...
export default function OrderEdit({ order, products, categories }: OrderEditProps) {
    ...
    const [selectedOrderType, setSelectedOrderType] = useState<OrderTypes | null>(
        order.type?.name as OrderTypes || OrderTypes.DINE_IN
    );
    const [errorMessage, setErrorMessage] = useState<string>('');
    const [showErrorDialog, setShowErrorDialog] = useState(false);
    const [processing, setProcessing] = useState(false);

    // Inicializar productos seleccionados desde la orden existente
    useEffect(() => {
        const initialProducts: SelectedProductItem[] = [];
        const initialNotes: Record<number, string> = {};

        order.details?.forEach(detail => {
            const product = products.find(p => p.id === detail.product_id);
            if (product) {
                // Agregar tantas instancias como cantidad tenga
                ...
                // Guardar la nota del producto
                if (detail.notes) {
                    initialNotes[product.id] = detail.notes;
                }
            }
        });
    ...
    const handleSubmit = (e: FormEvent) => {
        ...
        if (!selectedOrderType) {
            setErrorMessage('Debes seleccionar un tipo de orden');
            setShowErrorDialog(true);
            return;
        }
        ...
        router.put(`/dashboard/orders/${order.id}`, {
            ...
            onError: (errors) => {
                ...
                setErrorMessage(firstError || 'Error desconocido al actualizar el pedido');
                setShowErrorDialog(true);
                setProcessing(false);
            },
            onSuccess: () => {
        ...
        <AppLayout>
            <div className="grid grid-cols-[60%_40%] h-[calc(100vh-20px)] overflow-hidden">
                <div className="overflow-y-auto p-[30px] border-r border-gray-200">
                    <div className="flex items-center-safe justify-center mb-5 gap-[8px]">
                        <h3 className="nunito-bold text-[28px]">Editar Orden #{order.code}</h3>
            ...
            <Dialog open={showErrorDialog} onOpenChange={setShowErrorDialog}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle className="text-red-600">Error al actualizar pedido</DialogTitle>
